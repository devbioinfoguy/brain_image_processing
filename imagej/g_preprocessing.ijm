...
function processFiles(tiffFileDir,arrayOfFiles,tmpFileDir){
	...
	for(i=0; i<arrayLen; i++){
		...
		if(File.exists(tmpPath)){
			...
			// open image			
			open(path); rename("lowpass");
			open(path); rename("original");
			//run("TIFF Virtual Stack...","open="+path);
			// rename("new");

			// run FFT (or not)
			if(SFFT != 1){
				selectWindow("lowpass");
				bpstr= " filter_large=" + fftlp + " filter_small=" + ffthp + " suppress=None tolerance=5 process";
				run("Bandpass Filter...",bpstr);
				imageCalculator("Divide create 32-bit stack", "original","lowpass");
