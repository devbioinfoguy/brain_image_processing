function varargout = OrientTuning_GRATING_REPEAT(varargin)
% ORIENTTUNING_GRATING_REPEAT M-file for OrientTuning_GRATING_REPEAT.fig
%      ORIENTTUNING_GRATING_REPEAT, by itself, creates a new ORIENTTUNING_GRATING_REPEAT or raises the existing
%      singleton*.
%
%      H = ORIENTTUNING_GRATING_REPEAT returns the handle to a new ORIENTTUNING_GRATING_REPEAT or the handle to
%      the existing singleton*.
%

%      ORIENTTUNING_GRATING_REPEAT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ORIENTTUNING_GRATING_REPEAT.M with the given input arguments.
%
%      ORIENTTUNING_GRATING_REPEAT('Property','Value',...) creates a new ORIENTTUNING_GRATING_REPEAT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before OrientTuning_GRATING_REPEAT_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to OrientTuning_GRATING_REPEAT_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help OrientTuning_GRATING_REPEAT

% Last Modified by GUIDE v2.5 11-Feb-2013 14:43:13

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @OrientTuning_GRATING_REPEAT_OpeningFcn, ...
                   'gui_OutputFcn',  @OrientTuning_GRATING_REPEAT_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before OrientTuning_GRATING_REPEAT is made visible.
function OrientTuning_GRATING_REPEAT_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to OrientTuning_GRATING_REPEAT (see VARARGIN)

% Choose default command line output for OrientTuning_GRATING_REPEAT
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes OrientTuning_GRATING_REPEAT wait for user response (see UIRESUME)
...
function varargout = OrientTuning_GRATING_REPEAT_OutputFcn(hObject, eventdata, handles) 
...
for i = 1:SetupProp.Nframes
    ...
    if (i>SetupProp.NberFrameOnsetDelay)
        if InterTrialIteration<SetupProp.NbFrameInterTrial || Numberbar>numel(SetupProp.ChosenAngles)
            ...
            if k==SetupProp.NframeStim
                if Numberbar<numel(SetupProp.ChosenAngles)
                    LineFinished=1;
