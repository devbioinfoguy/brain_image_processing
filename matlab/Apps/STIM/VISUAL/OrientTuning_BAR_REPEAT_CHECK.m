function varargout = OrientTuning_BAR_REPEAT_CHECK(varargin)
% ORIENTTUNING_BAR_REPEAT_CHECK M-file for OrientTuning_BAR_REPEAT_CHECK.fig
%      ORIENTTUNING_BAR_REPEAT_CHECK, by itself, creates a new ORIENTTUNING_BAR_REPEAT_CHECK or raises the existing
%      singleton*.
%
%      H = ORIENTTUNING_BAR_REPEAT_CHECK returns the handle to a new ORIENTTUNING_BAR_REPEAT_CHECK or the handle to
%      the existing singleton*.
%

%      ORIENTTUNING_BAR_REPEAT_CHECK('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ORIENTTUNING_BAR_REPEAT_CHECK.M with the given input arguments.
%
%      ORIENTTUNING_BAR_REPEAT_CHECK('Property','Value',...) creates a new ORIENTTUNING_BAR_REPEAT_CHECK or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before OrientTuning_BAR_REPEAT_CHECK_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to OrientTuning_BAR_REPEAT_CHECK_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help OrientTuning_BAR_REPEAT_CHECK

% Last Modified by GUIDE v2.5 25-Feb-2013 13:45:37

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @OrientTuning_BAR_REPEAT_CHECK_OpeningFcn, ...
                   'gui_OutputFcn',  @OrientTuning_BAR_REPEAT_CHECK_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before OrientTuning_BAR_REPEAT_CHECK is made visible.
function OrientTuning_BAR_REPEAT_CHECK_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to OrientTuning_BAR_REPEAT_CHECK (see VARARGIN)

% Choose default command line output for OrientTuning_BAR_REPEAT_CHECK
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes OrientTuning_BAR_REPEAT_CHECK wait for user response (see UIRESUME)
...
% saved data.
    ...
    set(handles.MainColor,'Value',Settings.BarColorValue);
...
Settings.BarColorValue=get(handles.MainColor,'Value');
...
function varargout = OrientTuning_BAR_REPEAT_CHECK_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in MainColor.
function MainColor_Callback(hObject, eventdata, handles)
% hObject    handle to MainColor (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of MainColor
...
try  
    ...
    SetupProp.BarColor=get(handles.MainColor,'Value');
    ...
    Screen('FillRect',SetupProp.ScreenWindow,[0 0 0]);
    ...
    % We create the checkerboard texture
    ...
    [SetupProp.XScreen,SetupProp.YScreen]=meshgrid(XFactor*((SetupProp.ScreenRect(1):SetupProp.ScreenRect(3))),...
        YFactor*((SetupProp.ScreenRect(2):SetupProp.ScreenRect(4))));
    
    CheckX=0<=sign(cos(2*pi*SetupProp.XScreen*SetupProp.SpatialFreq));
    CheckY=0<=sign(cos(2*pi*SetupProp.YScreen*SetupProp.SpatialFreq));
       
    CheckOnMat(:,:,4)=255*uint8(xor(CheckX,CheckY));
    CheckOffMat(:,:,4)=255*uint8(~xor(CheckX,CheckY));
    
    SetupProp.CheckOn=Screen('MakeTexture', SetupProp.ScreenWindow, CheckOnMat);
    SetupProp.CheckOff=Screen('MakeTexture', SetupProp.ScreenWindow, CheckOffMat);
...
for i = 1:SetupProp.Nframes
    Screen('FillRect',SetupProp.ScreenWindow,0);
    ...
    if TrialIteration<=SetupProp.NbFramePreStim || TrialIteration>=SetupProp.NbFramePreStim+SetupProp.NbFrameStim+1
        ...
        if cos(2*pi*k*SetupProp.TempFreq/SetupProp.fps)>=0
            Screen('DrawTexture',SetupProp.ScreenWindow,SetupProp.CheckOn);
        else
            Screen('DrawTexture',SetupProp.ScreenWindow,SetupProp.CheckOff);
        end
        k=k+1;
    end
