function varargout = OrientTuning_GRATING_LOCKIN(varargin)
% ORIENTTUNING_GRATING_LOCKIN M-file for OrientTuning_GRATING_LOCKIN.fig
%      ORIENTTUNING_GRATING_LOCKIN, by itself, creates a new ORIENTTUNING_GRATING_LOCKIN or raises the existing
%      singleton*.
%
%      H = ORIENTTUNING_GRATING_LOCKIN returns the handle to a new ORIENTTUNING_GRATING_LOCKIN or the handle to
%      the existing singleton*.
%

%      ORIENTTUNING_GRATING_LOCKIN('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ORIENTTUNING_GRATING_LOCKIN.M with the given input arguments.
%
%      ORIENTTUNING_GRATING_LOCKIN('Property','Value',...) creates a new ORIENTTUNING_GRATING_LOCKIN or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before OrientTuning_GRATING_LOCKIN_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to OrientTuning_GRATING_LOCKIN_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help OrientTuning_GRATING_LOCKIN

% Last Modified by GUIDE v2.5 11-Feb-2013 15:56:22

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @OrientTuning_GRATING_LOCKIN_OpeningFcn, ...
                   'gui_OutputFcn',  @OrientTuning_GRATING_LOCKIN_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before OrientTuning_GRATING_LOCKIN is made visible.
function OrientTuning_GRATING_LOCKIN_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to OrientTuning_GRATING_LOCKIN (see VARARGIN)

% Choose default command line output for OrientTuning_GRATING_LOCKIN
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes OrientTuning_GRATING_LOCKIN wait for user response (see UIRESUME)
...
% saved data.
    ...
    set(handles.TempFreqMin,'String',Settings.TempFreqMinString);
    set(handles.TempFreqMax,'String',Settings.TempFreqMaxString);
    set(handles.PeriodStim,'String',Settings.PeriodStimString);
    ...
    set(handles.StimColor,'Value',Settings.StimColorValue);
...
Settings.TempFreqMinString=get(handles.TempFreqMin,'String');
Settings.TempFreqMaxString=get(handles.TempFreqMax,'String');
Settings.PeriodStimString=get(handles.PeriodStim,'String');
Settings.StimColorValue=get(handles.StimColor,'Value');


% --- Outputs from this function are returned to the command line.
function varargout = OrientTuning_GRATING_LOCKIN_OutputFcn(hObject, eventdata, handles) 
...
function TempFreqMin_Callback(hObject, eventdata, handles)
% hObject    handle to TempFreqMin (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of TempFreqMin as text
%        str2double(get(hObject,'String')) returns contents of TempFreqMin as a double


% --- Executes during object creation, after setting all properties.
function TempFreqMin_CreateFcn(hObject, eventdata, handles)
% hObject    handle to TempFreqMin (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
try
    ...
    SetupProp.TempFreqMin=str2double(get(handles.TempFreqMin,'String'));
    SetupProp.TempFreqMax=str2double(get(handles.TempFreqMax,'String'));
    ...
    SetupProp.PeriodStim=str2double(get(handles.PeriodStim,'String'));
        ...
        for i=1:5
            ...
            switch i
                ...
                case 5
                    ...
                    SpikeTraceData(CurrentNumberTrace+i).Label.YLabel='Stim time freq (Hz)';
                    SpikeTraceData(CurrentNumberTrace+i).Label.ListText='Stimulation freq';
            end
...
MeanTempFreq=(SetupProp.TempFreqMin+SetupProp.TempFreqMax)/2;
DistTempFreq=(SetupProp.TempFreqMax-SetupProp.TempFreqMin)/2;
xoffset = 0;
halfperiod=SetupProp.PeriodStim/2;
...
for i = 1:SetupProp.Nframes
    ...
    % cos based calculation
    %     TempFreq=MeanTempFreq+DistTempFreq*cos(2*(pi/SetupProp.fps)*(i-1)/SetupProp.PeriodStim);
    
    % Triangular based calculation
    time=(i-1)/SetupProp.fps;
    power=floor(time/halfperiod+1/2);
    
    TempFreq=MeanTempFreq+DistTempFreq*2/halfperiod*(time-halfperiod*power)*(-1)^power;
    ...
    RecordOutput.Stimulus(i)=TempFreq;
...
function TempFreqMax_Callback(hObject, eventdata, handles)
% hObject    handle to TempFreqMax (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of TempFreqMax as text
%        str2double(get(hObject,'String')) returns contents of TempFreqMax as a double


% --- Executes during object creation, after setting all properties.
function TempFreqMax_CreateFcn(hObject, eventdata, handles)
% hObject    handle to TempFreqMax (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function PeriodStim_Callback(hObject, eventdata, handles)
% hObject    handle to PeriodStim (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of PeriodStim as text
%        str2double(get(hObject,'String')) returns contents of PeriodStim as a double


% --- Executes during object creation, after setting all properties.
function PeriodStim_CreateFcn(hObject, eventdata, handles)
% hObject    handle to PeriodStim (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
