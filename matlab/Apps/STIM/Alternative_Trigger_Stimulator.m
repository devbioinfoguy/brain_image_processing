function varargout = Alternative_Trigger_Stimulator(varargin)
% ALTERNATIVE_TRIGGER_STIMULATOR M-file for Alternative_Trigger_Stimulator.fig
%      ALTERNATIVE_TRIGGER_STIMULATOR, by itself, creates a new ALTERNATIVE_TRIGGER_STIMULATOR or raises the existing
%      singleton*.
%
%      H = ALTERNATIVE_TRIGGER_STIMULATOR returns the handle to a new ALTERNATIVE_TRIGGER_STIMULATOR or the handle to
%      the existing singleton*.
%

%      ALTERNATIVE_TRIGGER_STIMULATOR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ALTERNATIVE_TRIGGER_STIMULATOR.M with the given input arguments.
%
%      ALTERNATIVE_TRIGGER_STIMULATOR('Property','Value',...) creates a new ALTERNATIVE_TRIGGER_STIMULATOR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Alternative_Trigger_Stimulator_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Alternative_Trigger_Stimulator_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Alternative_Trigger_Stimulator

% Last Modified by GUIDE v2.5 16-Jun-2013 22:01:15

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Alternative_Trigger_Stimulator_OpeningFcn, ...
                   'gui_OutputFcn',  @Alternative_Trigger_Stimulator_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Alternative_Trigger_Stimulator is made visible.
function Alternative_Trigger_Stimulator_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Alternative_Trigger_Stimulator (see VARARGIN)

% Choose default command line output for Alternative_Trigger_Stimulator
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Alternative_Trigger_Stimulator wait for user response (see UIRESUME)
...
function varargout = Alternative_Trigger_Stimulator_OutputFcn(hObject, eventdata, handles) 
...
try  
    ...
    FreqStim=str2double(get(handles.FreqStim,'String'));
    ...
    switch get(handles.TrigSettings,'Value')
        ...
        case 5
            % Set the port for receiving from NI card
            dio = digitalio('nidaq','Dev1');
            addline(dio,[1 2 3],1,'out'); % !!!!!!! for input line P0.1
            addline(dio,2,0,'in'); % !!!!!!! for input line P0.1
            TrigObject=dio;
        case 6
            % Set the port for receiving from NI card
...
%             We send TTL out and go on
        ...
        case 5
            ...
            while InLoop
                PortTrig=getvalue(TrigObject.Line(4));
            ...
            if CurrentState
                ...
                switch ParallelPortTrig
                    ...
                    case 4
                        % We do nothing
                    case 5
                        putvalue(TrigObject.Line([1 2 3]),[1 1 1]);
                    case 6
                        putvalue(TrigObject.Line([1 2 3]),[1 1 1]);
                end
            end
...
%                 set(handles.TrialsDone,'String',num2str(CurrentTrialNb));
                CurrentPhase=(cos(2*pi*FreqStim*(TimeTrial-PreStim)))>0;
                ...
                switch ParallelPortTrig
                    ...
                    case 5
                        if CurrentPhase
                            putvalue(TrigObject.Line([1 2 3]),[1 1 0]);
                        else
                            putvalue(TrigObject.Line([1 2 3]),[0 0 1]);
                        end
                    case 6
                        if CurrentPhase
                            putvalue(TrigObject.Line([1 2 3]),[1 1 0]);
...
function FreqStim_Callback(hObject, eventdata, handles)
% hObject    handle to FreqStim (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of FreqStim as text
%        str2double(get(hObject,'String')) returns contents of FreqStim as a double


% --- Executes during object creation, after setting all properties.
function FreqStim_CreateFcn(hObject, eventdata, handles)
% hObject    handle to FreqStim (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
