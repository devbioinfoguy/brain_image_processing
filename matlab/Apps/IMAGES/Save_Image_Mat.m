...
try
    ...
    if exist('matfile')==2
        ...
        % mechanism.
        SaveOldData=SpikeImageData;
        
        % At this stage, Matlab duplicate SpikeImageData in memory through
        % SaveOldData.
        SpikeImageData=SpikeImageData(ImageSel);
        
        if (get(handles.AppendFile,'Value')==1)
            save(FileSaving,'SpikeImageData','-append')
        else
            save(FileSaving,'SpikeImageData');
        end
        SpikeImageData=SaveOldData;
    end
