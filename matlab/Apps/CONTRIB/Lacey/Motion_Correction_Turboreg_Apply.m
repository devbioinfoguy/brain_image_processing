...
% Last Modified by GUIDE v2.5 13-Dec-2012 14:16:22
...
if (length(varargin)>1)
    ...
    set(handles.BlackEdgekeep,'Value',Settings.BlackEdgekeep);
    set(handles.NumTimesRepeat, 'String', Settings.NumTimesRepeatString);
    guidata(hObject, handles);
end
if ~isempty(SpikeMovieData)
    ...
    set(handles.ApplyMovieSelector,'String',TextMovie);
end
...
Settings.BlackEdgekeep=get(handles.BlackEdgekeep,'Value');
Settings.NumTimesRepeatString=get(handles.NumTimesRepeat, 'String');
...
if ~isfield(handles, 'MaskMotCorr')
    handles.MaskMotCorr = ones(size(SpikeMovieData(CurrentMovie).Movie(:,:,1)));
end
...
try
    ...
    CurrentMovie=get(handles.MovieSelector,'Value');
    MovieToApply=get(handles.ApplyMovieSelector, 'Value');
    if sum(size(SpikeMovieData(CurrentMovie).Movie)~=(size(SpikeMovieData(MovieToApply).Movie)))>0
        error('Movies are not the same size!')
    end
    SpikeMovieData(CurrentMovie).DataSize=size(SpikeMovieData(CurrentMovie).Movie);
    SpikeMovieData(MovieToApply).DataSize=size(SpikeMovieData(MovieToApply).Movie);
    ...
    switch get(handles.ReferenceSel,'Value')
        case 1
            RefPic=SpikeMovieData(CurrentMovie).Movie(:,:,str2double(get(handles.RefFrameNumber,'String')));
        case 2
            RefPic=imread(get(handles.FileRef,'String'));
        case 3
            RefPic=SpikeImageData(str2double(get(handles.ImageNumberRef, 'String'))).Image;
    end
    ...
    RefPic=10+single(RefPic);
    imgRegMask=ones(size(Mask),'single');
    AveragePictureEdge=(imgRegMask==0);
    
    numRepeats=str2double(get(handles.NumTimesRepeat, 'String'));
    
    % we add an offset to be able to give NaN to black borders
    for i=1:MaxFrame
        
        for rpt=1:numRepeats
            ToAlign=10+single(SpikeMovieData(CurrentMovie).Movie(:,:,i));
            ...
            SpikeMovieData(CurrentMovie).Movie(:,:,i)=cast(ImageOut-10,OriginalClass);
            ...
            translateMat=[0 0 0;0 0 0;ResultsOut.Translation(2) ResultsOut.Translation(1) 0];
            ...
            %rotationAngle=180/pi*ResultsOut.Rotation;
            SpikeMovieData(MovieToApply).Movie(:,:,i)=imtransform(SpikeMovieData(MovieToApply).Movie(:,:,i),tform,char(InterpList{InterpSelection}),...
                'UData',[1 SpikeMovieData(MovieToApply).DataSize(2)]-ResultsOut.Origin(2)-1,'VData',[1 SpikeMovieData(MovieToApply).DataSize(1)]-ResultsOut.Origin(1)-1,...
                'XData',[1 SpikeMovieData(MovieToApply).DataSize(2)]-ResultsOut.Origin(2)-1,'YData',[1 SpikeMovieData(MovieToApply).DataSize(1)]-ResultsOut.Origin(1)-1,...
                'FillValues',0);
        end
    ...
    if ~get(handles.BlackEdgekeep,'Value')
        % We project along z and make a nice binary image with 0 on the sides
        ...
        NewMovie=zeros([FinalNumRows FinalNumCols SpikeMovieData(CurrentMovie).DataSize(3)],class(SpikeMovieData(CurrentMovie).Movie));
        NewTime=zeros([FinalNumRows FinalNumCols SpikeMovieData(CurrentMovie).DataSize(3)],class(SpikeMovieData(CurrentMovie).TimePixel));
        
        for i=1:SpikeMovieData(CurrentMovie).DataSize(3)
            NewMovie(:,:,i)=imcrop(SpikeMovieData(CurrentMovie).Movie(:,:,i),RectCrop);
            NewTime(:,:,i)=imcrop(SpikeMovieData(CurrentMovie).TimePixel(:,:,i),RectCrop);
        ...
        SpikeMovieData(CurrentMovie).Xposition=imcrop(SpikeMovieData(CurrentMovie).Xposition,RectCrop);
        SpikeMovieData(CurrentMovie).Yposition=imcrop(SpikeMovieData(CurrentMovie).Yposition,RectCrop);
        SpikeMovieData(CurrentMovie).Zposition=imcrop(SpikeMovieData(CurrentMovie).Zposition,RectCrop);
        SpikeMovieData(CurrentMovie).Exposure=imcrop(SpikeMovieData(CurrentMovie).Exposure,RectCrop);
        
        
        
        for i=1:SpikeMovieData(MovieToApply).DataSize(3)
            NewMovie(:,:,i)=imcrop(SpikeMovieData(MovieToApply).Movie(:,:,i),RectCrop);
            NewTime(:,:,i)=imcrop(SpikeMovieData(MovieToApply).TimePixel(:,:,i),RectCrop);
        ...
        SpikeMovieData(MovieToApply).Movie=NewMovie;
        SpikeMovieData(MovieToApply).TimePixel=NewTime;
        SpikeMovieData(MovieToApply).DataSize=size(SpikeMovieData(MovieToApply).Movie);
        
        % We also adjust the single time matrix
        SpikeMovieData(MovieToApply).Xposition=imcrop(SpikeMovieData(MovieToApply).Xposition,RectCrop);
        SpikeMovieData(MovieToApply).Yposition=imcrop(SpikeMovieData(MovieToApply).Yposition,RectCrop);
        SpikeMovieData(MovieToApply).Zposition=imcrop(SpikeMovieData(MovieToApply).Zposition,RectCrop);
        SpikeMovieData(MovieToApply).Exposure=imcrop(SpikeMovieData(MovieToApply).Exposure,RectCrop);
...
% --- Executes on button press in BlackEdgekeep.
function BlackEdgekeep_Callback(hObject, eventdata, handles)
% hObject    handle to BlackEdgekeep (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of BlackEdgekeep



function ImageNumberRef_Callback(hObject, eventdata, handles)
% hObject    handle to ImageNumberRef (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of ImageNumberRef as text
%        str2double(get(hObject,'String')) returns contents of ImageNumberRef as a double


% --- Executes during object creation, after setting all properties.
function ImageNumberRef_CreateFcn(hObject, eventdata, handles)
% hObject    handle to ImageNumberRef (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function NumTimesRepeat_Callback(hObject, eventdata, handles)
% hObject    handle to NumTimesRepeat (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of NumTimesRepeat as text
%        str2double(get(hObject,'String')) returns contents of NumTimesRepeat as a double


% --- Executes during object creation, after setting all properties.
function NumTimesRepeat_CreateFcn(hObject, eventdata, handles)
% hObject    handle to NumTimesRepeat (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
