...
try
    ...
    for i=1:length(tracesToProcess)
        ...
        if size(thisTrace,1)==1
            thisTrace=[thisTrace(1)*ones(1,N), thisTrace, thisTrace(end)*ones(1,N)];
        else
            thisTrace=[thisTrace(1)*ones(N,1); thisTrace; thisTrace(end)*ones(N,1)];
        end
        newTrace=conv(thisTrace, gausswin(N,5/sigma), 'same');
        newTrace=newTrace(N+1:(end-N));
        if keepTraces
            saveInd=numTraces+i;
        ...
        SpikeTraceData(saveInd).Label.ListText=['smooth ', SpikeTraceData(traceInd).Label.ListText, ' ', num2str(sigma)];
    end
