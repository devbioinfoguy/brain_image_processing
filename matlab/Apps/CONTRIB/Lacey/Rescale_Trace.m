function varargout = Rescale_Trace(varargin)
% RESCALE_TRACE This App takes in a trace and sets it to 1 when it is
% above a maxvalue and 0 when it is less than or equal to the maxvalue.
% MaxValue can be set numerically or in terms of the mean and standard
% deviation of the trace, which will be calculated individually for each
% trace processed.
%
% Created by Lacey Kitch in 2012

% Edit the above text to modify the response to help Rescale_Trace

% Last Modified by GUIDE v2.5 29-Oct-2012 16:17:17

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Rescale_Trace_OpeningFcn, ...
                   'gui_OutputFcn',  @Rescale_Trace_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function Rescale_Trace_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Rescale_Trace (see VARARGIN)

% Choose default command line output for Rescale_Trace
...
if (length(varargin)>1)
    ...
    set(handles.MaxValue, 'String', Settings.MaxValueString);
    set(handles.MinValue, 'String', Settings.MinValueString);
end
...
Settings.MaxValueString=get(handles.MaxValue, 'String');
Settings.MinValueString=get(handles.MinValue, 'String');


% --- Outputs from this function are returned to the command line.
function varargout = Rescale_Trace_OutputFcn(hObject, eventdata, handles) 
...
try
    ...
    minValue=str2double(get(handles.MinValue, 'String'));
    ...
    for i=1:length(tracesToProcess)
        ...
        sortedTrace=sort(thisTrace,'ascend');
        traceMax=sortedTrace(round(98/100*length(thisTrace)));
        traceMin=sortedTrace(round(2/100*length(thisTrace)));
        thisTrace(thisTrace>traceMax)=traceMax;
        thisTrace(thisTrace<traceMin)=traceMin;
        newTrace=thisTrace-traceMin;
        newTrace=newTrace/max(newTrace)*maxValue+minValue;
        if keepTraces
            saveInd=numTraces+i;
        ...
        SpikeTraceData(saveInd).Label.ListText=['rescale ', SpikeTraceData(traceInd).Label.ListText];
    end
