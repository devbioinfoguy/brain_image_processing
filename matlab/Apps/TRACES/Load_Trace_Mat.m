...
try
    ...
    if exist('matfile')==2
        matObj = matfile(LocalFile);
        if ~isfield(matObj.SpikeTraceData, 'ID')
            thisData=matObj.SpikeTraceData(1,ListTraceToLoad);
            thisData(end).ID={};
            SpikeTraceData(BeginTrace:(BeginTrace-1+length(ListTraceToLoad)))=thisData;
            clear thisData
        else
        ...
        if ~isfield(Tmp.SpikeTraceData, 'ID')
            thisData=Tmp.SpikeTraceData(1,ListTraceToLoad);
            thisData(end).ID={};
            SpikeTraceData(BeginTrace:(BeginTrace-1+length(ListTraceToLoad)))=thisData;
            clear thisData
        else
